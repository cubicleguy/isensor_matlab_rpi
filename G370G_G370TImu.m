...
%
% Epson G370G, G370T IMU derived from Epson Device RPI handle class
% NOTE: For G370PDF1 & G370PDT0 support refer to G370F_G370SImu.m

classdef G370G_G370TImu < EpsonDeviceRpi
    ...
    methods
        ...
        function obj = G370G_G370TImu(speed, pin_reset, pin_drdy, IsPDT0)
        % class constructor
            if not(exist('speed', 'var'))
                fprintf('\nspeed not specified. Defaulting to 1000000 Hz');
                speed = 1000000;
            end
            if not(exist('pin_reset', 'var'))
                fprintf('\npin_reset not specified. Defaulting to pin 22');
                pin_reset = int8(22);
            end
            if not(exist('pin_drdy', 'var'))
                fprintf('\npin_drdy not specified. Defaulting to pin 24');
                pin_drdy = int8(24);
            end
            obj@EpsonDeviceRpi(speed, pin_reset, pin_drdy);
            if (exist('IsPDT0', 'var'))
                obj.IsPDT0 = IsPDT0;
            end
            obj.getDeviceCfg();
        ...
        % BytesPerBurst, FieldsPerBurst, FieldInBurst
            ...
            % Read sampling status
            obj.Sampling = obj.isSampling;
        ...
        % SIG_CTRL based on object properties
